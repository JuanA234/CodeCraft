    ...
    def tokenize(self):
        ...
        while source_index < len(source_code):
            ...
            if word == "bloque": 
                ...
                self.add_to_dict(tokens, "palabraReservadaPicoRotoSeguirPicando", word)
    
            # Reconocera un identificador y creara un indentificador para esta misma
            elif re.match('[a-z]', word) or re.match('[A-Z]', word):
                if '"' not in word:
                    if word[len(word)-1] == ";":
                        self.add_to_dict(tokens, "Identificador", word[0:len(word)-1])
                    else:   
                        self.add_to_dict(tokens, "Identificador", word)  
            #------------------------
            # Reconocera cuando empieza y termina las comillas identificadoras de un string, asignando el valor True a una variable, esto nos servirá para el elif a continuación
            elif '"' in word:
                dentroDeString = not dentroDeString
                
            # Reconocerá si la palabra está en un string gracias a la condicion anterior y creará un identificador como parte de un string
            elif dentroDeString == True:
                self.add_to_dict(tokens, "parteDeString", word)
